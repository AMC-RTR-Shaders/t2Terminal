...
{
	SAFE_LOG("Aeroplane shading language supported verison is : %s\n", glGetString(GL_SHADING_LANGUAGE_VERSION))
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Aeroplane Vertex Shader Compilation Log : %s\n", szInfoLog)
				SAFE_FREE(szInfoLog);			
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Aeroplane Fragment Shader Compilation Log : %s\n", szInfoLog)
				SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderProgramLinkStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Aeroplane Shader Program Link Log : %s \n", szInfoLog)
				SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
