...
LRESULT Windowing::OGLSurface::OGLSurfaceProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_SIZE:
		ReSize(LOWORD(lparam), HIWORD(lparam), _resizeAttributes);
		break;
	...
	while (_IsGameDone == false)
	{
		...
		else
		{
			if (_IsActiveWindow == true)
			{
				...
				_scene->Render(_hdc, _attributes);
			}
		}
	}
...
void Windowing::OGLSurface::ReSize(int width, int height, IScene::ResizeAttributes attributes)
{
	_width = width;
	_height = height;
	...
	_scene->ReSize(_width, _height, attributes);
}
