...
BOOL Utility::Quad::SceneHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	return 0;
}

void Utility::Quad::Initialize()
{
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Quad Vertex Shader Compilation Log : %s\n", szInfoLog)
					SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Quad Fragment Shader Compilation Log : %s\n", szInfoLog)
					SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderProgramLinkStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Quad Shader Program Link Log : %s \n", szInfoLog)
					SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
...
void Utility::Quad::ReSize(int width, int height, struct ResizeAttributes attributes)
{
	if (height == 0)
		height = 1;
...
void Utility::Quad::Render(HDC hdc, struct Attributes attributes)
{
...
void Utility::Quad::SceneTransition()
{
}

void Utility::Quad::UnInitialize()
{
