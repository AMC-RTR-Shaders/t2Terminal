...
void Harsh::SpotLight::Initialize()
{
	SAFE_LOG("SpotLight shading language supported verison is : %s\n", glGetString(GL_SHADING_LANGUAGE_VERSION))
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("SpotLight Fragment Shader Compilation Log : %s\n", szInfoLog)
					SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
...
BOOL Harsh::SpotLight::SceneHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	return 0;
}
...
void Harsh::SpotLight::ReSize(int width, int height, struct ResizeAttributes attributes)
{
	if (height == 0)
		height = 1;
...
void Harsh::SpotLight::Render(HDC hdc, struct Attributes attributes)
{
	...
	if(gbLight == true)
	{
		...
		lightPositionS1[0] = attributes.translateCoords[0] + 0.0f;
		lightPositionS1[1] = attributes.translateCoords[1] +3.0f;
		lightPositionS1[2] = attributes.translateCoords[2] + 0.0f;
		...
		lightPositionS2[0] = attributes.translateCoords[0] + 0.0f;
		lightPositionS2[1] = attributes.translateCoords[1] + 3.0f;
		lightPositionS2[2] = attributes.translateCoords[2] + 0.0f;
		...
		lightPositionS3[0] = attributes.translateCoords[0] + 0.0f;
		lightPositionS3[1] = attributes.translateCoords[1] + 3.0f;
		lightPositionS3[2] = attributes.translateCoords[2] + 0.0f;
	...
	translateMatrix = translate(attributes.translateCoords[0] + 0.0f, attributes.translateCoords[1] + -1.5f, attributes.translateCoords[2] + 0.0f);
	modelMatrix = translateMatrix*scaleMatrix;

	//rotationMatrix = rotate(angle_cube, 0.0f, 0.0f);
	...
	translateMatrix = translate(attributes.translateCoords[0] + 0.0f, attributes.translateCoords[1] + -1.0f, attributes.translateCoords[2] + 0.0f);
...
void Harsh::SpotLight::UnInitialize()
{
	if (Vao_cube)
	{
