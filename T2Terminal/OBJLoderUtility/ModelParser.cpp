...
void ThreeDModelLoader::ModelParser::parse(GLuint shaderProgramObject,PROCESS_TYPE processtype, DRAW_TYPE drawtype)
{
	int i = 0;
	...
	if (_draw_type == DRAW_TYPE::DRAW_USING_ARRAYS)
	{
		...
		_gl_object->initialize_for_draw_array_from_cpu(shaderProgramObject,_vertices_cnt, _pfvertices, _texcord_cnt, _pftexturesCoords, _normals_cnt, _pfnormals, _objLoader->getMaterialTable());
	...
	else
	{
		...
		_gl_object->initialize_for_draw_elements_from_cpu(shaderProgramObject,_vertices_cnt, _pfvertices, _texcord_cnt, _pftexturesCoords, _normals_cnt, _pfnormals, _position_index_cnt, _psPositionIndex, _objLoader->getMaterialTable());
		goto CLEAN_UP_MEMORY_AFTER_PUSHING_TO_GPU;
	}
...
{	
	...
	if (_objLoader->getVertices()->size() > 0)
	{
		triangle->vertex_positions.v1 = _objLoader->getFaces()->at(position).V1;
	...
	if (_objLoader->getTextureCoordinates()->size() > 0)
	{
		triangle->first_point_texture_coord.S = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T1 ).S;
		triangle->first_point_texture_coord.T = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T1 ).T;
		triangle->first_point_texture_coord.R = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T1 ).R;
		triangle->first_point_texture_coord.Q = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T1 ).Q;
	}
	if (_objLoader->getNormals()->size() > 0)
	{
		triangle->first_point_normals.N1 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N1 ).N1;
		triangle->first_point_normals.N2 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N1 ).N2;
		triangle->first_point_normals.N3 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N1 ).N3;
	}
	//
	...
	if (_objLoader->getVertices()->size()> 0)
	{
		triangle->vertex_positions.v2 = _objLoader->getFaces()->at(position).V2;
	...
	if (_objLoader->getTextureCoordinates()->size() > 0)
	{
		triangle->second_point_texture_coord.S = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T2 ).S;
		triangle->second_point_texture_coord.T = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T2 ).T;
		triangle->second_point_texture_coord.R = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T2 ).R;
		triangle->second_point_texture_coord.Q = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T2 ).Q;
	}
	if (_objLoader->getNormals()->size() > 0)
	{
		triangle->second_point_normals.N1 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N2 ).N1;
		triangle->second_point_normals.N2 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N2 ).N2;
		triangle->second_point_normals.N3 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N2 ).N3;
	}
	//
	...
	if (_objLoader->getVertices()->size() > 0)
	{
		triangle->vertex_positions.v3 = _objLoader->getFaces()->at(position).V3;
	...
	if (_objLoader->getTextureCoordinates()->size() > 0)
	{
		triangle->third_point_texture_coord.S = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T3 ).S;
		triangle->third_point_texture_coord.T = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T3 ).T;
		triangle->third_point_texture_coord.R = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T3 ).R;
		triangle->third_point_texture_coord.Q = _objLoader->getTextureCoordinates()->at(_objLoader->getFaces()->at(position).T3 ).Q;
	}
	if (_objLoader->getNormals()->size() > 0)
	{
		triangle->third_point_normals.N1 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N3 ).N1;
		triangle->third_point_normals.N2 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N3 ).N2;
		triangle->third_point_normals.N3 = _objLoader->getNormals()->at(_objLoader->getFaces()->at(position).N3 ).N3;
	}
	//
}
