...
#define SAFE_LOG(text){FILE *fp = NULL;fopen_s(&fp, "GL_LOG.txt", "a+");fprintf(fp,"%s\n",text);fclose(fp);fp=NULL;}

#define SAFE_SCENE_DELETE(scene) {if (scene){scene->UnInitialize();delete scene;scene = NULL;}}
