...
Praveen::Aeroplane::Aeroplane()
{
	_aeroplane_angle = 0.0f;

	_modelParser = new ModelParser("3DModels\\aeroplane\\aeroplane_1.obj");
...
BOOL Praveen::Aeroplane::SceneHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	return 0;
}

void Praveen::Aeroplane::Initialize()
{
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				sscanf(szInfoLog, "Aeroplane Vertex Shader Compilation Log : %s", szErrorfoLog);
				SAFE_LOG(szErrorfoLog)
				free(szErrorfoLog);
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				char *szErrorfoLog = (char*)malloc(infoLogLength + 1024);
				sscanf(szInfoLog, "Aeroplane Fragment Shader Compilation Log : %s\n", szErrorfoLog);
				SAFE_LOG(szErrorfoLog)
				free(szErrorfoLog);
	...
	glBindAttribLocation(_shaderProgramObject, VDG_ATTRIBUTE_POSITION, "vPosition");
	//PRE-LINK BINDING OF SHADER PROGRAM OBJECT WITTH TEXTURE SHADER ATTRIBUTE
	glBindAttribLocation(_shaderProgramObject, VDG_ATTRIBUTE_NORMAL, "vNormal");
	...
	//LOAD THE MODEL HERE
	CHECK_NULL(_modelParser);
	_modelParser->parse(_shaderProgramObject, ThreeDModelLoader::PROCESS_TYPE::CPU, ThreeDModelLoader::DRAW_TYPE::DRAW_USING_ARRAYS);
...
void Praveen::Aeroplane::Update()
{
	_aeroplane_angle = _aeroplane_angle + 0.3f;

	if (_aeroplane_angle >= 360.0f)
		_aeroplane_angle = _aeroplane_angle - 360.0f;
}

void Praveen::Aeroplane::ReSize(int width, int height)
{
	if (height == 0)
		height = 1;
...
void Praveen::Aeroplane::Render(HDC hdc)
{
	...
	rotateMatrix = vmath::rotate<GLfloat>(_aeroplane_angle, _aeroplane_angle, _aeroplane_angle);
...
void Praveen::Aeroplane::SceneTransition()
{
}

void Praveen::Aeroplane::UnInitialize()
{
