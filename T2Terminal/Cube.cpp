...
BOOL Utility::Cube::SceneHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	return 0;
}

void Utility::Cube::Initialize()
{
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Cube Vertex Shader Compilation Log : %s\n", szInfoLog)
				SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Cube Fragment Shader Compilation Log : %s\n", szInfoLog)
				SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderProgramLinkStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("Cube Shader Program Link Log : %s \n", szInfoLog)
				SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
...
void Utility::Cube::ReSize(int width, int height, struct ResizeAttributes attributes)
{
	if (height == 0)
		height = 1;
...
void Utility::Cube::Render(HDC hdc, struct Attributes attributes)
{
...
void Utility::Cube::SceneTransition()
{
}

void Utility::Cube::UnInitialize()
{
