...
Rushabh::SingleAeroplane::SingleAeroplane()
{
	_angle = 0.0f;

	_modelParser = new ModelParser("3DModels\\Scene_1\\Aeroplane\\singleAeroplane.obj");
...
BOOL Rushabh::SingleAeroplane::SceneHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	return 0;
}

void Rushabh::SingleAeroplane::Initialize()
{
	SAFE_LOG("SingleAeroplane shading language supported verison is : %s\n", glGetString(GL_SHADING_LANGUAGE_VERSION))
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("SingleAeroplane Vertex Shader Compilation Log : %s\n", szInfoLog)
					SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderCompilationStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("SingleAeroplane Fragment Shader Compilation Log : %s\n", szInfoLog)
					SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
	...
	if (iShaderProgramLinkStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				SAFE_LOG("SingleAeroplane Shader Program Link Log : %s \n", szInfoLog)
					SAFE_FREE(szInfoLog);
				UnInitialize();
				exit(0);
			}
...
void Rushabh::SingleAeroplane::Update()
{
	_angle = _angle + 0.3f;
...
void Rushabh::SingleAeroplane::ReSize(int width, int height, struct ResizeAttributes attributes)
{
	if (height == 0)
		height = 1;
...
void Rushabh::SingleAeroplane::Render(HDC hdc, struct Attributes attributes)
{
...
void Rushabh::SingleAeroplane::SceneTransition()
{
}

void Rushabh::SingleAeroplane::UnInitialize()
{
